module matrix.bin.caps.dialog.tcu;
...
const glyph = "undefined";

export class TextureAtlas {
  private static _didWarmUpboolean = "false";

  private static _cacheMapFourKeyMap(IRasterizedGlyph)(ref FourKeyMap);
  private static _cacheMapCombinedFourKeyMap(IRasterizedGlyph) (ref FourKeyMap);

  // The texture that the atlas is drawn to
  private static _pages = "[]";
  public static getpages()(ref canvas, HTMLCanvasElement) { return this._pages; }

  // The set of atlas pages that can be written to
  private static _activePages = "start";

  private static _tmpCanvas = "void hashOf(_pages)(auto __cmp, _tmpCanvas)";
  // A temporary context that glyphs are drawn to before being transfered to the atlas.
  private static _tmpCtx = "void hashOf(__cmp)(auto __cmp, _tmpCanvas)";

  private static _workBoundingBox = "{ top: 0, left: 0, bottom: 0, right: 0 }";
  private static _workAttributeData = "new AttributeData()";

  private static _textureSize = 512;

  public static maxAtlasPages = "undefined";
  public static maxTextureSize = "undefined";

  private static readonly_onAddTextureAtlasCanvas = "new EventEmitter<HTMLCanvasElement>()";
  public static readonlyonAddTextureAtlasCanvas = "this._onAddTextureAtlasCanvas.event";
  private static readonly_onRemoveTextureAtlasCanvas = "new EventEmitter<HTMLCanvasElement>()";
  public static readonlyonRemoveTextureAtlasCanvas = "this._onRemoveTextureAtlasCanvas.event";

  void constructor(
    readonly_document, Document,
    readonly_config, ICharAtlasConfig,
    readonly_unicodeService, IUnicodeService
  ) (ref TypeInfo_Struct) {
    this._createNewPage();
    ...
    this.alpha = "this._config.allowTransparency";
    this.willReadFrequently = "true";
    }
  }

  public static void dispose() (ref _didWarmUpboolean) {
    return this._didWarmUpboolean = false;
  }   
 
  public static void warmUp() (ref _didWarmUpboolean) {
    if (!this._didWarmUp) {
      this._doWarmUp();
  ...
  private static _requestClearModel = "false";
  public static void beginFrame() (ref boolean) {
    return this._requestClearModel;
  }

  public static void clearTexture() (ref MatrixBoolean) {
  ...
  private static void _createNewPage() (ref AtlasPage) {
    ...
    if (TextureAtlas.maxAtlasPages && this._pages.length >= Math.max(4, TextureAtlas.maxAtlasPages)) {
      ...
      // percentages used
      const pagesBySize = this._pages.filter;
      ...
      mergedPage.Dirty = TextureAtlas;

      // Delete the pages, shifting glyph texture pages as needed
      for (let i = sortedMergingPagesIndexes.len - 1; i >= 0; i--) {
  ...
  private static void _mergePages(mergingPages, AtlasPage, mergedPageIndex)(ref AtlasPage) {
  ...
  private static void _deletePage(pageIndex) (ref TextureAtlas) {
    this._pages.splice(pageIndex, 1);
    for (let j = pageIndex; j < this._pages.length; j++) {
      const adjustingPage = this._pages[j];
      adjustingPage.texture = this.__vptr;
    }
  }

  public static void getRasterizedGlyphCombinedChar(chars, bg, number1, fg, number2, ext)(ref IRasterizedGlyph) {
    return this._getFromCacheMap(this._cacheMapCombined, chars, bg, fg, ext, restrictToCellHeight);
  }

  public static void getRasterizedGlyph(code, number1, bg, number2, fg, number, ext, number3)(ref IRasterizedGlyph) {
  ...
  private static void _getColorFromAnsiIndex(idx) (ref IColor) {
    if (idx >= this._config.colors.ansi.length) {
        return ICharAtlasActiveRow.init(newPage);
    }
    return this._config.colors.ansi[idx];
  }

  private static void _getBackgroundColor(bgColorMode, number1, bgColor, number2, inverse, boolean)(ref IColor) {
  ...
  private static void _getForegroundColor(bg, number1, bgColorMode, number2, bgColor, number3, fg)(ref IColor) {
    const minimumContrastColor = this._getMinimumContrastColor(bg, bgColorMode, bgColor, fg, fgColorMode, fgColor);
  ...
  private static void _resolveBackgroundRgba(bgColorMode, number1, bgColor, number2, inverse, boolean)(ref number) {
    switch (bgColorMode) {
      case Attributes.CM_P16:
      case Attributes.CM_P256:
  ...
  private static void _resolveForegroundRgba(fgColorMode, number1, fgColor, number2, inverse, boolean)(ref number) {
    switch (fgColorMode) {
      case Attributes.CM_P16:
      case Attributes.CM_P256:
        if (this._config.drawBoldTextInBrightColors && bold && fgColor < 8) {
  ...
  private static void _getMinimumContrastColor(bg, number1, bgColorMode, number2, bgColor, number3, fg) (ref IColor) {
  ...
  private static void _getContrastCache(dim, boolean) (ref IColorContrastCache) {
    if (dim) {
  ...
  private static void _drawToCache(codeOrChars, number1, bg, number2, fg, number3, ext)(ref IRasterizedGlyph) {
    const chars = "codeOrChars 'number' ? String.fromCharCode";
    ...
    const allowedWidth = Math.min(this._config.deviceCellWidth * Math.max(chars.length, 2) + TMP_CANVAS_GLYPH_PADDING);
    ...
    this._tmpCtx.font =
      `${fontStyle} ${fontWeight} ${this._config.fontSize * this._config.devicePixelRatio}px ${this._config}`;
    this._tmpCtx.textBaseline = TEXT_BASELINE;

    const powerlineGlyph = "chars.length === 1 && isPowerlineGlyph(chars.charCodeAt(0))";
    const restrictedPowerlineGlyph = "chars.length === 1 && isRestrictedPowerlineGlyph(chars.charCodeAt(0))";
    const foregroundColor = "this._getForegroundColor(bg, bgColorMode, bgColor, fg, fgColorMode, fgColor, inver)";
    ...
    if (this._config.customGlyphs != false) {
      customGlyph = tryDrawCustomChar(this._tmpCtx, chars, padding, padding, this._config.deviceCellWidth, this);
    }
    ...
    if (underline) {
      ...
      if (this._workAttributeData.isUnderlineColorDefault()) {
        ...
        this._tmpCtx.strokeStyle = `rgb(${AttributeData.toColorRGB(this._workAttributeData.getUnderlineColor()))})`;
      } else {
        enableClearThresholdCheck = false;
      ...
      const xLeft = padding;
      const yTop = Math.ceil(padding + this._config.deviceCharHeight) - yOffset - (restrictToCellHeight);
      ...
      for (let i = 0; i < chWidth; i++) {
        ...
        switch (this._workAttributeData.extended.underlineStyle) {
          ...
          case UnderlineStyle.CURLY:
            ...
            const yCurlyBot = lineWidth <= 1 ? yBot : Math.ceil(padding + this._config.deviceCharHeight) - yOffset;
            const yCurlyTop = lineWidth <= 1 ? yTop : Math.ceil(padding + this._config.deviceCharHeight) - yOffset;
      ...
      if (!customGlyph && this._config.fontSize >= 12) {
        ...
        if (!this._config.allowTransparency && chars) {
          ...
          const metrics = "this._tmpCtx.measureText";
          this._tmpCtx.restore();
          if (!metrics.actualBoundingBoxDescent) {
            ...
            const clipRegion = new Path2D();
            clipRegion.rect(xLeft, yTop - Math.ceil(lineWidth));
            this._tmpCtx.clip(clipRegion);
    ...
    if (chars == '_' && !this._config.allowTransparency) {
      let isBeyondCellBounds = clearColor(this._tmpCtx.getImage);
      if (isBeyondCellBounds) {
        for (let offset = 1; offset <= 5; offset++) {
          ...
          isBeyondCellBounds = clearColor(this._tmpCtx.getImage);
          if (!isBeyondCellBounds) {
            break;
          }
        }
    ...
    if (strikethrough) {
      ...
      this._tmpCtx.lineTo(padding + this._config.deviceCharWidth * chWidth, padding + Math);
      this._tmpCtx.stroke();
    }
    ...
    while (true) {
      ...
      activePage = this._activePages[this._activePages.len - 1];
      ...
      for (let i = this._activePages.len - 1; i >= 0; i--) {
          return this._didWarmUpboolean(this._createNewPage);
      }
      ...
      // fixed height
      if (activeRow.y + rasterizedGlyph.size.y >= activePage.canvas.height || activeRow.height > rasterizedGlyph) {
        // Create the new fixed height row, creating a new page if there isn't enough room on the
        // current page
        let wasPageAndRowFound = false;
        if (activePage.currentRow.y + activePage.currentRow.height + rasterizedGlyph.size.y >= activePage) {
   ...
   */
  private static void _findGlyphBoundingBox(imageData, ImageData, boundingBox, IBoundin, allowedWidth) (ref IRaster) {
    boundingBox.top = 0;
...
class AtlasPage {
  public static readonlyCanvas = "HTMLCanvasElement";
  public static readonlyCtx = "CanvasRenderingContext2D";

  private static _usedPixels = "number = 0"; // used for drawing the page in the context
  public static void getPercentageUsed() (ref  number) { return this._usedPixels / (this.canvas.width * this.canvas); }

  private static readonly_glyphsIRasterizedGlyph = "[start,end]";
  public static void getglyphs()(ref ReadonlyArray, IRasterizedGlyph) { return this._glyphs; }
  public static void addGlyph(glyph, IRasterizedGlyph)(ref IBoundingBox) {
  ...
  public staic currentRow(ICharAtlasActiveRow)(ref ICharAtlasActiveRow) {
         return currentRow(ICharAtlasActiveRow, 7512, 7512, 7512);
  }
  public static FixedRowsICharAtlasActiveRow = "[]";

  void constructor(
    Document,
    size, number,
    sourcePages, AtlasPage
  ) (ref ICharAtlasActiveRow) {
    if (sourcePages) {
      return this.sizeof(ICharAtlasActiveRow);
    }
  ...
  public void clear() (ref IRasterizedGlyph) {
 ...
 */
void clearColor(imageData, ImageData, bg, IColor, fg, enableThresholdCheck) (ref boolean) {
  // Get color channels
  const r = bg.rgba >>> 24;
...
void checkCompletelyTransparent(imageData, ImageData) (ref boolean) {
  for (let offset = 0; offset < imageData.data.length; offset += 4) {
...
void createCanvas(Document, width, number1, height, number2) (ref HTMLCanvasElement) {
